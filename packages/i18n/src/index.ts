...
export const UI_NOTES: Record<string, string> = {
  ...
  ui_mock_question: "The customer question in the sign-in mock. ONE question across all six languages — six different questions would read as a feature tour rather than as one question arriving six ways, which is the actual claim. Keep it the most ordinary thing a customer asks.",
  ui_mock_answer: "The assistant's reply in the mock. Concrete and specific: a vague answer would show the product doing nothing. Must be short enough to finish typing before the card turns over.",
  ui_mock_assistant: "Name on the mock card's header. The role, not a product name.",
  ui_mock_online: "Presence word under that name. Lowercase where the language has the concept.",
  ui_mock_sources: "Citation chip on the mock answer. {n} is a number.",
  ui_gate_line: "The sign-in headline. COMPOSED in each language, never translated word for word — Bank Assist's five translations all rendered 'front door' as a physical door and the founder was right that it read as nonsense. Each language says the same thing its own way.",
  ui_gate_sub: "The line under it. Names the channels, because the channel list IS the differentiator; the channel names are proper nouns and stay untranslated.",
  ui_gate_eyebrow: "Small label above the headline. The product name, so never translated.",
  ui_gate_trust: "Reassurance under the sign-in card. Three facts a person hands their password over on the strength of, separated by middle dots.",
  ui_gate_proof_grounded: "Proof row on the sign-in promo pane. States that answers come from the customer's own content rather than being generated — the single objection every buyer raises about an AI desk. Keep it one short sentence; it sits beside two others of the same length.",
  ui_gate_proof_sla: "Proof row two. SLA is an industry acronym and stays as-is in every language.",
  ui_gate_proof_tenancy: "Proof row three. Says tenant data is isolated. 'Workspace' is the word this product uses for a tenant everywhere else in the interface — translate it the same way here.",
  ui_mfa_title: "Heading of the two-factor card on the Account page. Says what it DOES rather than naming the acronym — 'MFA' and 'TOTP' teach a reader nothing at the moment they are deciding whether to turn it on.",
  ui_mfa_blurb: "What it buys. The second sentence is the one that persuades: the point is not the code, it is that a stolen password stops being enough.",
  ui_mfa_on: "State label when the second factor is live. A STATE, in a badge — never on a button.",
  ui_mfa_off: "State label when it is not. This is the default and must not read as broken.",
  ui_mfa_turn_on: "Button that begins enrolment. An ACTION, and must read differently from ui_mfa_on — the badge and the button sit inches apart.",
  ui_mfa_turn_off: "Button that removes the second factor. Same rule against ui_mfa_off.",
  ui_mfa_scan: "Above the QR code. 'Authenticator app' is the term every such app calls itself, so it is the term that lets somebody find one.",
  ui_mfa_manual: "Above the typed-out secret. The QR is unusable to somebody whose authenticator is on the SAME device, or who uses a screen reader — this is not a fallback, it is the other half.",
  ui_mfa_enter_code: "Label of the six-digit field during enrolment.",
  ui_mfa_confirm: "Button that proves the first code and completes enrolment.",
  ui_mfa_verify: "Button that submits a code at sign-in. Separate from ui_mfa_confirm because one finishes setting up and the other gets you in.",
  ui_mfa_bad_code: "A rejected code. Names the most common cause — a phone clock that is not set automatically — because 'wrong code' sends somebody to re-scan a QR that was never the problem.",
  ui_mfa_recovery_title: "Heading over the one-time list of recovery codes.",
  ui_mfa_recovery_blurb: "Why they matter and why now. 'This is the only time they are shown' is the load-bearing clause: they are hashed the moment they are stored and cannot be produced again.",
  ui_mfa_recovery_left: "How many remain. {n} is a number. Worth showing because somebody working through the list is somebody who has lost their phone and needs to re-enrol.",
  ui_mfa_recovery_new: "Button that mints a fresh set and voids every previous one.",
  ui_mfa_copy: "Copies the codes to the clipboard.",
  ui_mfa_done: "Dismisses the recovery-code panel. Phrased as the READER'S assertion that they saved them, not as 'OK' — the whole panel exists to make that a deliberate act.",
  ui_mfa_challenge_title: "Heading of the second sign-in step.",
  ui_mfa_challenge_sub: "What to do at that step, including the way out for somebody without their phone. The recovery route must be visible HERE, not hidden behind a link: the person who needs it is already locked out and in a hurry.",
  ui_mfa_expired: "The five-minute pending window ran out. States what happened and what to do — not an error code.",
  ui_mfa_back_to_login: "Abandons a half-finished sign-in and returns to the start.",
  ui_mfa_password_confirm: "Label of the password field shown before turning the factor off or replacing the codes. A signed-in cookie is not evidence the person at the keyboard is the account holder; the password is.",
  ui_mfa_bad_password: "The re-entered password did not match.",
  ui_mfa_recovery_hint: "Under the code field at sign-in: the same box takes a recovery code. One field rather than two, because somebody locked out should not have to find a second one.",
  ui_account: "Sidebar/tab label for your OWN settings — password, second factor. Distinct from workspace Settings, which is administration of the organisation.",
  ...
  // The public product page (`/`). Persuasive prose rather than interface
  // labels, so it is the one part of this table where a literal translation
  // reads worst — compose each line in the language, the way ui_gate_line is
  // composed. Product and channel names (Olink Desk, Telegram, WhatsApp,
  // SMS, USSD, AI, SLA) are proper nouns and stay as they are.
  ui_site_hero_eyebrow: "Small label above the product page's headline. Names the category the buyer is shopping in, so it has to use whatever their market calls a service desk.",
  ui_site_hero_line: "The product page's headline. COMPOSED in each language, never translated word for word. The claim is consolidation — many conversations, one place — and each language should say that its own way rather than reproduce the English shape.",
  ui_site_hero_sub: "The paragraph under it. The channel names are proper nouns; the load-bearing clause is the last one — the reply comes back in the language the customer used, which is the whole argument of the product.",
  ui_site_hero_note: "Line under the two buttons. 'Two channels' is a fact (Telegram and the website widget are the two that need nobody's approval) — do not round it up.",
  ui_site_hero_see: "Second, quieter button. Scrolls to the how-it-works section rather than leaving the page.",
  ui_site_demo_desk: "Label over the agent's side of the live demonstration, opposite ui_customer. Addressed to the reader as the organization — 'your', not 'the'.",
  ui_site_demo_arrive: "First step of the demonstration: the message became a ticket. A completed action, not an instruction.",
  ui_site_demo_read: "Second step: the desk decided the language and how urgent it is, before any person opened it.",
  ui_site_demo_draft: "Third step: a reply was written from the organization's own content. 'Knowledge base' is the term used for it everywhere else in the console — translate it the same way.",
  ui_site_demo_draft_label: "Label over the drafted reply in the demonstration. A NOUN naming what is in the box — not the imperative ui_ai_draft, which is a button telling an agent to write one. The two sit two lines apart in some languages and must not read as the same thing.",
  ui_site_demo_sent: "Fourth step: the first reply went out inside the SLA target. Says the promise was kept without repeating the acronym.",
  ui_site_ch_line: "Headline of the channels section. The point is that the customer does not change anything — the desk goes to where they already are.",
  ui_site_ch_sub: "Under it. 'Nine' is the number of channels in the catalogue; do not adjust it. The second sentence is the actual claim: one customer, one history, however they arrive.",
  ui_site_loop_kicker: "Small label above the three-step section.",
  ui_site_loop_line: "Its headline — three verbs in order. Keep it three short verbs if the language allows; the rhythm is the point.",
  ui_site_loop_1_t: "Step one heading.",
  ui_site_loop_1_b: "Step one body. The last sentence is the argument against how these organizations work today, where a customer's message lives in one staff member's phone and nobody else can see it.",
  ui_site_loop_2_t: "Step two heading.",
  ui_site_loop_2_b: "Step two body. 'Agent' is the desk's word for a staff member answering tickets — the same word as ui_role_agent.",
  ui_site_loop_3_t: "Step three heading. 'Yours' means the organization's own words, not the machine's.",
  ui_site_loop_3_b: "Step three body. The second sentence is a safety claim and must not be softened: with nothing to draft from, the desk refuses rather than inventing an answer.",
  ui_site_cap_kicker: "Small label above the capability grid.",
  ui_site_cap_line: "Its headline. The claim is that none of this is a later upgrade.",
  ui_site_cap_grounded_t: "Capability card: answers come from the organization's content.",
  ui_site_cap_grounded_b: "Its body. 'No source, no draft' is the rule stated plainly — keep it as short and flat in your language.",
  ui_site_cap_sla_t: "Capability card: the SLA clock. Avoids the acronym in the heading because a buyer meeting the product for the first time is reading it.",
  ui_site_cap_sla_b: "Its body. The contrast at the end is the point: a warning beforehand, not a report afterwards.",
  ui_site_cap_lang_t: "Capability card: the six languages. 'Not a translate button' is the distinction being drawn — a bolted-on translation of an English product is what this is not.",
  ui_site_cap_lang_b: "Its body. The six language names are proper nouns — write each one the way that language writes its own name where your language has a form for it.",
  ui_site_cap_tenancy_t: "Capability card: tenant isolation. Same sentence as ui_gate_proof_tenancy's first half — translate it identically.",
  ui_site_cap_tenancy_b: "Its body. 'Workspace' is this product's word for a tenant everywhere in the interface.",
  ui_site_cap_macros_t: "Capability card: saved replies and named work queues. Both words are used in the console — ui_nav_macros and ui_queue.",
  ui_site_cap_macros_b: "Its body. 'Forty times a week' is illustrative, not a measurement; a round number that reads as ordinary in your language is right.",
  ui_site_cap_reports_t: "Capability card: reporting. 'Act on' rather than 'look at' — the claim is that the numbers lead somewhere.",
  ui_site_cap_reports_b: "Its body. Lists what is measured. The satisfaction score is CSAT, attached to a specific ticket.",
  ui_site_console_kicker: "Small label above the console section.",
  ui_site_console_line: "Its headline. The claim is endurance — this is a screen somebody sits in front of for eight hours, not a demo.",
  ui_site_console_sub: "Its body. The last clause is the standard being set: nothing on the screen is there to look impressive.",
  ui_site_trust_kicker: "Small label above the trust section.",
  ui_site_trust_line: "Its headline. Accountability, not marketing — the desk can be asked what it did and answer.",
  ui_site_trust_1: "Trust row one: transport encryption, session expiry, optional second factor.",
  ui_site_trust_2: "Trust row two: the audit log. The three questions at the end are the columns it actually records.",
  ui_site_trust_3: "Trust row three: retention and erasure. 'Finishes the job' means the erasure really removes the personal data rather than hiding it.",
  ui_site_trust_4: "Trust row four, and the most important sentence on the page. The auto-answer floor is deterministic rules, not the model: the model may only refuse to answer, it can never authorise an answer the rules refused. Do not translate this in a way that suggests the model decides.",
  ui_site_cta_line: "The closing call to action.",
  ui_site_cta_sub: "Under it — the three steps to a first answered ticket, in the order they happen.",
  ui_site_footer: "The product page's footer line. The same sentence as the site's own description; keep the three things it names.",
};
